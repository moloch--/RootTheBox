...
class CorporationApiHandler(BaseHandler):
    ...
    def get(self, id: str = None):
        if id is None or id == "":
            data = {
                "corporations": [
                    corporation.to_dict() for corporation in Corporation.all()
                ]
            }
        else:
    ...
    def delete(self, id: str):
        ...
        if corporation is not None:
            ...
            dbsession.commit()
            self.write(json.dumps({"message": "Corporation deleted"}))
        else:
            self.write(json.dumps({"message": "Corporation not found"}))
