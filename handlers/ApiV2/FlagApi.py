...
class FlagApiHandler(BaseHandler):
    ...
    def get(self, id=None):
        if id is None or id == "":
            data = {"flags": [flag.to_dict() for flag in Flag.all()]}
        else:
            flag = Flag.by_id(id)
            if flag is not None:
                data = {"flag": flag.to_dict()}
            else:
    ...
    def post(self, *args, **kwargs):
        ...
        if "box" not in data:
            data = {"box": None, "message": "Box is required"}
            self.write(json.dumps(data))
            return

        if Box.by_name(data["box"]) is None:
            data = {"box": data["box"], "message": "This box does not exist"}
            self.write(json.dumps(data))
            return
