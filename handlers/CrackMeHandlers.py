...
class CrackMeHandler(RequestHandler):
        ...
        if user != None:
            if(user.team.crack_me.token == token):
                ...
                user_action = Action(
                    ...
                    description=unicode("%s successfully cracked the level %s" %
                                        (user.display_name, level)),
    ...
    def notify(self, user, crack_me):
        ...
        message = unicode("%s successfully cracked the level %s Crack Me" %
                          (user.display_name, crack_me.id))
    ...
    def get(self, *args, **kwargs):
        ...
        if session != None:
            ...
            if user.team.crack_me != None:
                filePath = self.application.settings[
                    'crack_me_dir'] + '/' + user.team.crack_me.uuid
                current = open(filePath, 'rb')
                data = current.read()
                current.close()
                self.set_header('Content-Type', user.team.crack_me.content)
                self.set_header('Content-Disposition',
                                'attachment; filename=%s' % user.team.crack_me.file_name)
