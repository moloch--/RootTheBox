...
class ApplicationTest(AsyncHTTPTestCase):
    ...
    def __fetch(self, path, **kw):
        ...
        if auth:
            ...
            try:
                # posting to /login in order to get the auth cookie. if login
                # fails, treats as unauthed
                self.post('/login', data={'user_name': auth[0],
                                          'password': auth[1]}, follow_redirects=False)(self.stop)
                auth_cookie = '%s;' % self.wait(
